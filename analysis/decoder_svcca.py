...
def get_representation(model, dictionary, src_lang, src_lang_id, tgt_lang, batch_size, layer_wise, layer_num=0):
    ...
    for i in range(batch_num):
        ...
        batch_src_texts = [torch.cat([torch.LongTensor([(src_lang_id + DICT_LENGTH)]),
                                      dictionary.encode_line(text, add_if_not_exist=False, append_eos=True)], dim=0) for
                           text in batch_src_texts]
        batch_src_texts = collate_tokens(batch_src_texts, pad_idx=1, left_pad=True).cuda()

        batch_tgt_texts = tgt_texts[start:end]
        batch_tgt_texts = [dictionary.encode_line(text, add_if_not_exist=False, append_eos=True) for text in
                           batch_tgt_texts]
        ...
        decoder_out = model.decoder(batch_tgt_texts, encoder_out=encoder_out, src_lengths=batch_src_texts.shape[0],
...
model = \
    TransformerModel.from_pretrained(checkpoint, checkpoint_file="checkpoint.pt", data_name_or_path=data_path).models[0]
model.eval().cuda()
