...
for i, tgt in enumerate(language_sequence):
    ...
    for j, src in enumerate(language_sequence):
        if i == j:
            continue
        path_ref = os.path.join(root_path, "opus_scripts", "results", method, str(model_id), f"{src}-{tgt}.r")
        path_hypo = os.path.join(root_path, "opus_scripts", "results", method, str(model_id), f"{src}-{tgt}.h")
...
file = open(
    os.path.join(root_path, "opus_scripts", "results", method, str(model_id), f"{str(model_id)}.bertscore"),
    'w', encoding='utf-8')
